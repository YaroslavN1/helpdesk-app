...
- **`MAX_TICKET_FROM_NAME_LENGTH` / `MAX_TICKET_SUBJECT_LENGTH` / `MAX_TICKET_BODY_LENGTH` / `MAX_TICKET_HTML_BODY_LENGTH`** (`core/src/constants/ticket.ts`, currently `255` / `255` / `5000` / `20000`) — bound the fields `inboundEmailSchema` accepts from inbound email. Unlike `createReplySchema`'s `.max()`, these are enforced via `.transform()` (a silent truncate), not `.max()` (a rejection) — see Webhooks API below for why.
